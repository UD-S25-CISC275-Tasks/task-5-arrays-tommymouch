...
export function allRGB(colors: string[]): boolean {
    return colors.length === 0 || colors.every(color => ["red", "blue", "green"].includes(color));
}
...
export function makeMath(addends: number[]): string {
    ...
    return `${sum}=${addends.length > 0 ? addends.join("+") : "0"}`;
}
...
export function injectPositive(values: number[]): number[] {
    ...
    const sum = values.slice(0, index === -1 ? values.length : index).reduce((acc, num) => acc + num, 0);
    return index === -1 ? [...values, sum] : [...values.slice(0, index + 1), sum, ...values.slice(index + 1)];
}
